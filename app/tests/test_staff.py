...
class TestStaff(unittest.TestCase):
    ...
    def test_add(self):
        ...
        self.assertEqual(name, 'Fally Ipupa') 

    # Test empty name
    def test_add_empty_name(self):
        self.assertEqual(self.staff.add_staff(''), 'Too short name')

    #Test name longer than expected (Longer than 20 characters)
    def test_add_too_long_name(self):
        name = 'Ezekiel Adebayo Mabukoroje'
        self.assertEqual(self.staff.add_staff(name), 'Too long name') 

       #Type of name passed
    def test_add_list_type_name(self):
        self.assertEqual(self.staff.add_staff(['Fally Ipupa']), 'Invalid name type')
